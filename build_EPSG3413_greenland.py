...
# =======================
# FIXME: Centralize data location vars so users only have to edit this
#  info once
lc_epsg = Path(
    "data", "EPSG3413", "EPSG3413grid.json"
)  # NOTE: created by plot_grids.py
# lc_bamber = "data/BamberDEM/Greenland_bedrock_topography_V3.nc"
# lc_seaRise = "data/SeaRise/Greenland1km.nc"
# lc_racmo2p0 = "data/RACMO2.3/smb_1km_GrIS_downscaled_RACMO2_3.nc"
...
volume.add_argument("-q", "--quiet", action="store_true", help="Run silently")
...
if not args.use_template:
    ...
    nc_base, base = epsg3413.build_base(f_base, f_epsg, 1000.0)
    ...
    searise.bheatflx_artm_epsg3413(
        args, nc_seaRise, nc_base, base, proj_epsg3413, proj_eigen_gl04c
    )
    speak.notquiet(args, "   Done!")
    nc_seaRise.close()
    ...
    csatho.dhdt_all(args, nc_csatho, nc_base, base, proj_epsg3413)
    speak.notquiet(args, "   Done!")
    nc_csatho.close()
    ...
    # NOTE: the space in front of altitude so argparse doesn't
