...
nc_new.variables["thk"].source = (
    "Antarctica-1km.BISICLES.CISM-style.nc; generated by Dan Martin (or maybe "
    "Steph Cornford)."
)

nc_new.variables["thk"].reference = (
    "Fretwell, P., et al.: Bedmap2: improved ice bed, surface and thickness "
...
nc_new.variables["thkerr"].reference = (
    "Fretwell, P., et al.: Bedmap2: improved ice bed, surface and thickness "
    "datasets for Antarctica, The Cryosphere, 7, 375-393, "
    "https://doi.org/10.5194/tc-7-375-2013, 2013. "
)
nc_new.variables["thkerr"].comments = (
    "Resampled from 5km grid using 2D nearest neighbor interpolation, missing "
    "values filled with a value of 150.0. "
)
if not FILL:
...
nc_new.variables["bheatflx"].comments = (
    "Resampled from 15km grid using 2D nearest neighbor interpolation; polar "
    "sterographic projection true scaled latitude not specified in dataset -- "
    "asumed -71 deg. (EPSG:3031)"
)
...
nc_new.variables[
    ...
    "upward_geothermal_heat_flux_at_ground_level_in_land_ice standard_error"
)
...
nc_new.history = "Created {} by Joseph H. Kennedy.".format(
...
nc_new.variables[
    "acab"
].comments = "Mean 1979--2010 SMB; 2D linear interpolation of 27 km dataset."
...
nc_new.variables[
    "artm"
].comments = "Mean 1979--2010 t2m; 2D linear interpolation of 27 km dataset."
...
nc_new.variables[
    "topg"
].comments = "Resampled from 5km grid using 2D nearest neighbor interpolation"
...
nc_new.variables["vx"].reference = (
    ...
    "Ice Sheet, Science. 333. 1427-1430. "
    "https://doi.org/10.1126/science.1208336. Mouginot J., B. Scheuchl and "
    "E. Rignot (2012), Mapping of Ice Motion in Antarctica Using "
    "Synthetic-Aperture Radar Data, Remote Sensing, doi 10.3390/rs4092753"
)
...
nc_new.variables["dhdt"].comments = (
    "Uncertainty estimates over the ice sheet proper, exlcuding the pole-hole "
    "and ice shelves, represent the std deviation of the difference between "
    "CryoSat and airborn altimetry. Over the pole-hole and ice-shelves where "
    "data is missing, we've doubled the uncertainty. "
)
