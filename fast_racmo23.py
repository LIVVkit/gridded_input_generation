...
nc_racmo = ncfunc.get_nc_file(
    "data/RACMO2.3/monthly/smb.1958-2013.BN_1958_2013.MM.nc", "r"
)
...
y_1km = np.arange(bamber["ll"][1], bamber["ur"][1] + 1000.0, 1000.0)
x_1km = np.arange(bamber["ll"][0], bamber["ur"][0] + 1000.0, 1000.0)
new.y = y_1km[::km_reso]
new.x = x_1km[::km_reso]
...
nc_new, new = projections.grid_center_latlons(
    nc_new, new, eigenGl04c, proj_name
)

nc_new.createVariable(proj_name, "c")
nc_new.variables[proj_name].grid_mapping_name = "polar_stereographic"
nc_new.variables[proj_name].straight_vertical_longitude_from_pole = 321.0
nc_new.variables[proj_name].latitude_of_projection_origin = 90.0
nc_new.variables[proj_name].standard_parallel = 71.0
nc_new.variables[proj_name].false_easting = 0.0
nc_new.variables[proj_name].false_northing = 0.0
nc_new.variables[proj_name].scale_factor_at_projection_origin = 1.0
nc_new.variables[proj_name].units = "m"
nc_new.variables[
    proj_name
].proj4_string = "+proj=stere +lat_ts=71.0 +lat_0=90 +lon_0=321.0 +k_0=1.0"

racmo.xs, racmo.ys = eigenGl04c(
    racmo.lon_grid[:, :].ravel(), racmo.lat_grid[:, :].ravel()
)  # pyproj is gross and does (lon, lat) for in and out. Barf.
...
for ii in range(racmo.time[:].shape[0]):
    new.smb[ii, :, :] = scipy.interpolate.griddata(
        list(zip(racmo.ys, racmo.xs)),
        racmo.smb[ii, :, :].ravel(),
