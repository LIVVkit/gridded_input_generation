...
def bheatflx_artm_epsg3413(
    ...
    for base_var, sea_var in list(base_vars.items()):
        speak.verbose(
            args,
            "   Interpolating "
            + sea_var
        ...
        seaRise_interp = scipy.interpolate.RectBivariateSpline(
            seaRise.y[:], seaRise.x[:], sea_data, kx=1, ky=1, s=0
        ...
        for ii in range(0, base.nx):
            ...
            sys.stdout.flush()
            base_bamber[:, ii] = seaRise_interp.ev(
        ...
        base_y_mask = np.ma.masked_outside(
            base2bamber.y_grid, seaRise.y[0], seaRise.y[-1]
        )
        base_x_mask = np.ma.masked_outside(
            base2bamber.x_grid, seaRise.x[0], seaRise.x[-1]
        )
        base_masked = np.ma.masked_array(
...
def bheatflx_artm_bamber(args, nc_seaRise, nc_base, base):
    ...
    seaRise_data[
        seaRise_y_equal_base : seaRise_y_equal_base + seaRise_ny,
        seaRise_x_equal_base : seaRise_x_equal_base + seaRise_nx,
    ] = -seaRise_bheatflx[
        0, :, :
    ]  # invert sign!
