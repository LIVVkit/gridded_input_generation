...
def mcb_epsg3413(
    ...
    for ii in range(0, base.nx):
        ...
        sys.stdout.flush()
        base.thk[:, ii] = massCon_to_base.ev(
    ...
    for var, var_list in base_vars.items():
        ...
        sys.stdout.write("         [%-60s] %d%%" % ("=" * 0, 0.0))
        sys.stdout.flush()
        massCon2bamber = projections.transform(
            massCon, proj_epsg3413, proj_eigen_gl04c
        )
        ...
        for ii in range(0, massCon.nx):
            ...
            sys.stdout.flush()
            massCon_bamber[:, ii] = sec_interp.ev(
                massCon2bamber.y_grid[:, ii], massCon2bamber.x_grid[:, ii]
            )
        sys.stdout.write("\r         [%-60s] %d%%\n" % ("=" * 60, 100.0))
        sys.stdout.flush()
        ...
        for ii in range(0, base.nx):
            ...
            sys.stdout.flush()
            base_bamber[:, ii] = sec_interp.ev(
                base2bamber.y_grid[:, ii], base2bamber.x_grid[:, ii]
            )
        sys.stdout.write("\r         [%-60s] %d%%\n" % ("=" * 60, 100.0))
        sys.stdout.flush()
        ...
        speak.verbose(
            args, "         Interpolating combined dataset to base grid."
        )
        sys.stdout.write("         [%-60s] %d%%" % ("=" * 0, 0.0))
        sys.stdout.flush()
        ...
        base_y_mask = np.ma.masked_outside(
            base.y_grid, massCon.y[0], massCon.y[-1]
        )
        base_x_mask = np.ma.masked_outside(
            base.x_grid, massCon.x[0], massCon.x[-1]
        )
        base_mcb_masked = np.ma.masked_array(
            base_mcb, mask=np.logical_or(base_y_mask.mask, base_x_mask.mask)
        )

        base_bamber[~base_mcb_masked.mask] = base_mcb_masked[
            ~base_mcb_masked.mask
        ]
        ...
        bam_shape = shape(
            {"type": "polygon", "coordinates": [list(zip(bamx, bamy))]}
        )
        base_pts_in_bamber = list(
            zip(base2bamber.x_grid.flatten(), base2bamber.y_grid.flatten())
        )
...
def mcb_bamber(
    ...
    for ii in tqdm(range(0, base.ny), desc="Y loop"):
        for jj in tqdm(range(0, base.nx), desc="X loop", leave=False):
            ...
            if (
                trans.x_grid[ii, jj] < massCon.x_grid[0, 0]
            ...
            # interpolate!
            alpha = (trans.y_grid[ii, jj] - massCon.y_grid[nn_ii, nn_jj]) / (
                massCon.dy * i_s
            )
            beta = (trans.x_grid[ii, jj] - massCon.x_grid[nn_ii, nn_jj]) / (
                massCon.dx * j_s
            )
    ...
    temp_x_grid, temp_y_grid, temp_data = pyproj.transform(
        proj_epsg3413,
        proj_eigen_gl04c,
        trans.x_grid.flatten(),
        trans.y_grid.flatten(),
        new_data.flatten(),
    )
