...
def add_time_and_shrink(
    ...
    time.comment = (
        ...
        "Joughin's 2015 InSAR data. Because this is a synthesis of datasets "
        "across many time periods, the inital date is inherently fuzzy and "
        "should be changed to suit your purposes."
    )
    ...
    for skip in coarse_list:
        ...
        for var_name, var_data in nc_base.variables.items():
            if var_name not in ["time", "x1", "y1", "lat", "lon", proj_name]:
                var_coarse = nc_coarse.createVariable(
                    var_name, "f4", ("time", "y1", "x1",)
                )
