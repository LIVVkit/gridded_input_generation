...
def main(args):
    ...
    lon_grid, lat_grid = proj_eigen_gl04c(
    ...
    base.ll_y = base.y_grid.flatten() - base.dy / 2.0
    base.ll_x = base.x_grid.flatten() - base.dx / 2.0

    base.lr_y = base.y_grid.flatten() - base.dy / 2.0
    base.lr_x = base.x_grid.flatten() + base.dx / 2.0

    base.ur_y = base.y_grid.flatten() + base.dy / 2.0
    base.ur_x = base.x_grid.flatten() + base.dx / 2.0

    base.ul_y = base.y_grid.flatten() + base.dy / 2.0
    base.ul_x = base.x_grid.flatten() - base.dx / 2.0

    # FIXME: Pick projection based on input file!
    base.ll_lon, base.ll_lat = proj_eigen_gl04c(
        base.ll_x, base.ll_y, inverse=True
    )
    base.lr_lon, base.lr_lat = proj_eigen_gl04c(
        base.lr_x, base.lr_y, inverse=True
    )
    base.ur_lon, base.ur_lat = proj_eigen_gl04c(
        base.ur_x, base.ur_y, inverse=True
    )
    base.ul_lon, base.ul_lat = proj_eigen_gl04c(
        base.ul_x, base.ul_y, inverse=True
    )

    base.corner_lat = np.column_stack(
        (base.ll_lat, base.lr_lat, base.ur_lat, base.ul_lat)
    )
    base.corner_lon = np.column_stack(
        (base.ll_lon, base.lr_lon, base.ur_lon, base.ul_lon)
    )
    ...
    proj_aea = projections.equal_area(
        min_lat, max_lat, (max_lon + min_lon) / 2.0
    )
    ...
    for ii in range(base.N):
        ...
        points = {"type": "polygon", "coordinates": [list(zip(x, y))]}
    ...
    base.y0 = (base.y[:-1] + base.y[1:]) / 2.0
    base.x0 = (base.x[:-1] + base.x[1:]) / 2.0
    base.y0_grid, base.x0_grid = scipy.meshgrid(
        base.y0[:], base.x0[:], indexing="ij"
    )
    base.dim0 = (base.ny - 1, base.nx - 1)
    base.N0 = base.dim0[0] * base.dim0[1]

    # FIXME: Pick projection based on input file!
    lon0_grid, lat0_grid = proj_eigen_gl04c(
        base.x0_grid.ravel(), base.y0_grid.ravel(), inverse=True
    )
    ...
    base.ll_y0 = base.y0_grid.flatten() - base.dy / 2.0
    base.ll_x0 = base.x0_grid.flatten() - base.dx / 2.0

    base.lr_y0 = base.y0_grid.flatten() - base.dy / 2.0
    base.lr_x0 = base.x0_grid.flatten() + base.dx / 2.0

    base.ur_y0 = base.y0_grid.flatten() + base.dy / 2.0
    base.ur_x0 = base.x0_grid.flatten() + base.dx / 2.0

    base.ul_y0 = base.y0_grid.flatten() + base.dy / 2.0
    base.ul_x0 = base.x0_grid.flatten() - base.dx / 2.0

    # FIXME: Pick projection based on input file!
    base.ll_lon0, base.ll_lat0 = proj_eigen_gl04c(
        base.ll_x0, base.ll_y0, inverse=True
    )
    base.lr_lon0, base.lr_lat0 = proj_eigen_gl04c(
        base.lr_x0, base.lr_y0, inverse=True
    )
    base.ur_lon0, base.ur_lat0 = proj_eigen_gl04c(
        base.ur_x0, base.ur_y0, inverse=True
    )
    base.ul_lon0, base.ul_lat0 = proj_eigen_gl04c(
        base.ul_x0, base.ul_y0, inverse=True
    )

    base.corner_lat0 = np.column_stack(
        (base.ll_lat0, base.lr_lat0, base.ur_lat0, base.ul_lat0)
    )
    base.corner_lon0 = np.column_stack(
        (base.ll_lon0, base.lr_lon0, base.ur_lon0, base.ul_lon0)
    )
    ...
    proj_aea0 = projections.equal_area(
        min_lat0, max_lat0, (max_lon0 + min_lon0) / 2.0
    )
    ...
    for ii in range(base.N0):
        ...
        points0 = {"type": "polygon", "coordinates": [list(zip(x0, y0))]}
    ...
    climo.lon_grid = nc_climo.createVariable(
        "lon", "f4", ("time", "y1", "x1",)
    )
    ...
    climo.lat_grid = nc_climo.createVariable(
        "lat", "f4", ("time", "y1", "x1",)
    )
    ...
    climo.lon0_grid = nc_climo.createVariable(
        "lon0", "f4", ("time", "y0", "x0",)
    )
    ...
    climo.lat0_grid = nc_climo.createVariable(
        "lat0", "f4", ("time", "y0", "x0",)
    )
    ...
    climo.area[0, :, :] = (
        base.area[:, :] / projections.SURFACE_AREA_EARTH
    ) * projections.SQR_DEG_ON_SPHERE
    climo.area.units = "square degrees"

    climo.area0 = nc_climo.createVariable("area0", "f4", ("time", "y0", "x0",))
    climo.area0[0, :, :] = (
        base.area0[:, :] / projections.SURFACE_AREA_EARTH
    ) * projections.SQR_DEG_ON_SPHERE
