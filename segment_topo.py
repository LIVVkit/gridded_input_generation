...
def parse_args(args=None):
    ...
    volume.add_argument(
        "-q", "--quiet", action="store_true", help="Run silently"
    )
...
def main(args):
    ...
    speak.verbose(
        args, "    Number of distinct shallow regions found: {}".format(ids)
    )

    mask = np.array(segments * -1.0)
    # Set ocean
    ...
    shape_gl = shape(
        {"type": "polygon", "coordinates": [list(zip(gl_y, gl_x))]}
    )
    ...
    speak.progress(
        args, mask.shape[0], mask.shape[0], width=60, char="=", indent=4
    )
    ...
    if args.write:
        ...
        base.comment = (
            "Mask values: -3, shallow ocean or land outside paleo domain; "
            + "-2, bare paleo land; "
