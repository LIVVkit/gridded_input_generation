...
def main(args):
    ...
    base.ll_y = base.y_grid.flatten(order="C") - base.dy / 2.0
    base.ll_x = base.x_grid.flatten(order="C") - base.dx / 2.0
    base.lr_y = base.y_grid.flatten(order="C") - base.dy / 2.0
    base.lr_x = base.x_grid.flatten(order="C") + base.dx / 2.0
    base.ur_y = base.y_grid.flatten(order="C") + base.dy / 2.0
    base.ur_x = base.x_grid.flatten(order="C") + base.dx / 2.0
    base.ul_y = base.y_grid.flatten(order="C") + base.dy / 2.0
    base.ul_x = base.x_grid.flatten(order="C") - base.dx / 2.0
    ...
    scrip.dims.note = (
        "The grid dims are flipped (from how they appear in input dataset) because "
    ...
    scrip.center_lat = nc_scrip.createVariable(
        "grid_center_lat", "f4", ("grid_size",)
    )
    scrip.center_lat[:] = base.lat_grid[:, :].flatten(order="C")
    scrip.center_lat.setncattr("units", "degrees")

    scrip.center_lon = nc_scrip.createVariable(
        "grid_center_lon", "f4", ("grid_size",)
    )
    scrip.center_lon[:] = base.lon_grid[:, :].flatten(order="C")
    scrip.center_lon.setncattr("units", "degrees")

    scrip.corner_lat = nc_scrip.createVariable(
        "grid_corner_lat", "f4", ("grid_size", "grid_corners",)
    )
    scrip.corner_lat[:, :] = base.corner_lat[:, :]
    scrip.corner_lat.units = "degrees"

    scrip.corner_lon = nc_scrip.createVariable(
        "grid_corner_lon", "f4", ("grid_size", "grid_corners",)
    )
