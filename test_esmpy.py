...
lc_epsg = (
    "data/EPSG3413/EPSG3413grid.json"  # NOTE: created by plot_projections.py
)
lc_epsg_shr = (
    "data/EPSG3413/EPSG3413grid_shrunk.json"  # NOTE: created by plot_grids.py
)
...
parser.add_argument(
    "-e", "--extended", help="Produce the extended grid.", action="store_true"
)
...
insar_grid = ESMF.Grid(
    filename=os.path.join(os.getcwd(), epsg_InSAR),
...
cism_grid = ESMF.Grid(
    filename=os.path.join(os.getcwd(), epsg_scrip),
    filetype=ESMF.FileFormat.SCRIP,
)
...
lon_grid, lat_grid = proj_epsg3413(
